...
client.on("messageCreate", async (message) => {
  ...
  if (content.startsWith("!playnhac") || content.startsWith("!pn")) {
    ...
    if (isYoutubeUrl) {
      ...
      const embed = new EmbedBuilder()
        .setDescription(
          `<:youtube:1243493337302962196> Đã ᴛʜêᴍ ɴʜạᴄ: [_${videoTitle}_](${videoUrl})\n \n \n` +
            `<:heart:1269216504381968455> ᴛạɪ ᴋêɴʜ: _${videoAuthor}_\n` +
            `<:heart:1269216504381968455> 𝚃𝚑ờ𝚒 𝙻ượ𝚗𝚐: _${Math.floor(videoDuration / 60)} ᴘʜúᴛ : ${videoDuration % 60} ɢɪâʏ_\n` +
            `<:heart:1269216504381968455> 𝚈ê𝚞 𝙲ầ𝚞 𝙱ở𝚒: <@${message.author.id}>`,
        )
