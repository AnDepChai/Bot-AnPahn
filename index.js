...
events.EventEmitter.defaultMaxListeners = 15; // Số lượng listeners tối đa muốn đặt, ví dụ là 15
...
const allowedChannel = ["1239079798907605092",             "1255417420810555392",             "1266316948141244447"];
...
client.on("messageCreate", async (message) => {
  if (message.content === "!uptime") {
    ...
    const uptimeEmbed = new EmbedBuilder()
      ...
      .addFields(
        { name: "Ngày", value: `0`, inline: true },
        { name: "Giờ", value: `0`, inline: true },
        { name: "Phút", value: `0`, inline: true },
        { name: "Giây", value: `0`, inline: true },
      )
      .setTimestamp();

    const uptimeMessage = await message.channel.send({ embeds: [uptimeEmbed] });
    client.uptimeMessage = uptimeMessage;
  }
});

function updateUptime() {
  if (!client.uptimeMessage) return;
  ...
  client.uptimeMessage.edit({ embeds: [embed] });
}
...
client.on("messageCreate", async (message) => {
  ...
  if (command === "!setafk") {
    const reason = args.join(" ") || "ᴋʜôɴɢ ᴄó ʟý ᴅᴏ!";
    afkUsers.set(message.author.id, reason);

    const embed = new EmbedBuilder()
      .setColor("#00FF00")
      .setTitle(`<@${message.author.id}> | Đã sᴇᴛ ᴀғᴋ!`)
      .setDescription(`ʟý ᴅᴏ: ${reason}`);
  ...
  if (afkUsers.has(message.author.id)) {
    ...
    const embed = new EmbedBuilder()
      .setColor("#00FF00")
      .setTitle(`<@${message.author.id}> | ᴄʜàᴏ ᴍừɴɢ ᴛʀở ʟạɪ ᴋêɴʜ ᴄʜᴀᴛ!`)
      .setDescription("ᴛôɪ đã xᴏá ᴛʀạɴɢ ᴛʜáɪ ᴀғᴋ ᴄủᴀ ʙạɴ.");
  ...
  if (message.mentions.users.size > 0) {
    message.mentions.users.forEach((user) => {
      if (afkUsers.has(user.id)) {
        ...
        const embed = new EmbedBuilder()
          .setColor("#FF0000")
          .setTitle(`ɴɢườɪ ᴅùɴɢ ɴàʏ đᴀɴɢ ᴀғᴋ ᴠᴜɪ ʟòɴɢ đợɪ...`)
          .setDescription(`ʟý ᴅᴏ: ${reason}`);
...
client.on("messageCreate", async (message) => {
  ...
  if (content.startsWith("!ip")) {
    const embed = new EmbedBuilder()
      .setTitle("IP Server Minecraft")
      .setColor("#00ff00")
      .addFields(
        { name: "PC:", value: "```MineATH.Fun:1561```" },
        {
          name: "PE:",
          value: "```MineATH.Fun``` | ```Port: 1561```",
        },
      )
...
client.on("messageCreate", async (message) => {
  ...
  if (
    ...
    if (!userVoiceChannel) {
      const embed = new EmbedBuilder()
        .setDescription(
          "<a:loadingma:1265977725559111710> ʙạɴ ᴘʜảɪ ᴠàᴏ ᴠᴏɪᴄᴇ ᴄʜᴀɴɴᴇʟ ᴛʀướᴄ ᴋʜɪ ᴅùɴɢ ʟệɴʜ ɴàʏ!",
        )
        .setColor("#ff0000");
      await message.channel.send({ embeds: [embed] });
      return;
  ...
  if (content.startsWith("!playnhac") || content.startsWith("!pn")) {
    ...
    if (isYoutubeUrl) {
      ...
      const embed = new EmbedBuilder()
        .setDescription(
          `<:youtube:1243493337302962196> Đã ᴛʜêᴍ ɴʜạᴄ: _${videoTitle}_\n \n` +
            `• ᴛáᴄ ɢɪả: _${videoAuthor}_\n` +
            `• ᴛʜờɪ ʟượɴɢ: _${Math.floor(videoDuration / 60)}p:${videoDuration % 60}s_\n` +
            `• ʏêᴜ ᴄầᴜ ʙởɪ: <@${message.author.id}>`,
        )
